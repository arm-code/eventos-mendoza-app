...
// ─────────────────────────────────────────────────────────────────────────────
// PrintEventContractDocument — Nodo de exportación a imagen/PDF.
// Siempre 794px de ancho con tamaños de fuente fijos (desktop).
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#5b21b6', textTransform: 'uppercase' }}>{business.name}</div>
          <div style={{ fontSize: 13, color: '#6b6577', marginTop: 4 }}>Renta de Mobiliario para Eventos · Contrato y Nota de Servicio</div>
          {business.phone && <div style={{ fontSize: 13, color: '#6b6577' }}>Tel / WhatsApp: {business.phone}</div>}
          <div style={{ fontSize: 13, color: '#6b6577' }}>Ciudad Juárez, Chihuahua</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div
          ...
          <div style={{ fontSize: 13, color: '#6b6577' }}>Fecha: {event.date ? formatDate(event.date) : 'Por definir'}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div style={{ backgroundColor: '#faf9fc', borderRadius: 8, padding: 16 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: '#1a1626', textTransform: 'uppercase' }}>{clientName}</div>
          {clientPhone && <div style={{ fontSize: 13, color: '#6b6577', marginTop: 4 }}>Teléfono: {clientPhone}</div>}
          {event.noteFolio && <div style={{ fontSize: 13, color: '#7c3aed', fontWeight: 600, marginTop: 6 }}>Nota vinculada: {event.noteFolio}</div>}
        </div>
        <div style={{ backgroundColor: '#faf9fc', borderRadius: 8, padding: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#7c3aed', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 }}>Ubicación y Servicio</div>
          <div style={{ fontSize: 13, color: '#1a1626', fontWeight: 600, textTransform: 'uppercase' }}>{event.eventAddress || 'Dirección de evento no especificada'}</div>
          <div style={{ fontSize: 13, color: '#6b6577', marginTop: 6, textTransform: 'uppercase' }}>
            <span style={{ fontWeight: 600, color: '#453f52' }}>Garantía:</span>{' '}
            {event.guaranteeDocument || event.contract?.guaranteeDocument || 'INE / Credencial de Elector'}
          </div>
      ...
      {/* Detalle del servicio */}
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#7c3aed', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 }}>Descripción del Servicio Prestado</div>
        <div style={{ border: '1px solid #ececf2', borderRadius: 8, padding: 16, fontSize: 14, lineHeight: 1.5, color: '#1a1626', minHeight: 80, backgroundColor: '#ffffff', textTransform: 'uppercase' }}>
          {event.serviceDescription || event.name || event.notes || 'Renta de mobiliario y equipo para evento.'}
        </div>
      </div>
      ...
      {/* Cláusula */}
      <div style={{ backgroundColor: '#faf9fc', border: '1px solid #e9d5ff', borderRadius: 8, padding: 16, marginBottom: 40, fontSize: 11, lineHeight: 1.6, color: '#4b5563' }}>
        <div style={{ fontWeight: 700, color: '#5b21b6', marginBottom: 6, fontSize: 12 }}>CLÁUSULA DE RESPONSABILIDAD Y GARANTÍA DE MOBILIARIO</div>
        El cliente manifiesta recibir el mobiliario y equipo en óptimas condiciones de uso y funcionamiento.
        El cliente se compromete a cuidar y devolver el equipo en la fecha convenida. En caso de daño, pérdida,
        destrucción o extravío del mobiliario, el cliente se obliga a cubrir el costo total de reparación o reposición
        inmediata. Para garantía del cumplimiento de este contrato, el cliente hace entrega en custodia del documento
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, marginTop: 60, textAlign: 'center' }}>
        <div>
          <div style={{ borderTop: '1px solid #a78bfa', paddingTop: 8, fontSize: 12, fontWeight: 600, color: '#1a1626' }}>Firma de Conformidad del Cliente</div>
          <div style={{ fontSize: 11, color: '#6b6577', marginTop: 2, textTransform: 'uppercase' }}>{clientName}</div>
        </div>
        <div>
          <div style={{ borderTop: '1px solid #a78bfa', paddingTop: 8, fontSize: 12, fontWeight: 600, color: '#1a1626' }}>Por Eventos Mendoza</div>
          <div style={{ fontSize: 11, color: '#6b6577', marginTop: 2 }}>Firma Autorizada</div>
        </div>
      </div>
